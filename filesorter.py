...
class FileOrganizer:
    def __init__(self, root):
        ...
        self.directory = None #initially no directory selected
        self.undo_data = [] #empty list to track the files that are moved (undo operation)
        self.hash_to_paths = {}
        self.duplicate_groups = []
        self.current_group_index = 0
        self.log_path = None
        self.config = self.load_config()  # app-level default at startup

        select_button = Button(root, text="Select Directory", command=self.select_directory) 
        select_button.pack(pady=20) #20 pixels vertical padding
    ...
    def open_categories_editor(self):
        cfg = json.loads(json.dumps(self.config))  # shallow copy
    ...
    def select_all_but_first(self):
        # Clear and select all rows except the first in the group
    ...
    def delete_selected(self):
        ...
        def normalize_path(p):
            # Ensure a clean absolute path, remove Windows '\\?\' prefix if present
            if not p:
                return p
            p = str(p)
            if p.startswith('\\\\?\\'):
        ...
        for raw_path in selected:
            ...
            try:
                ...
                try:
                    ...
                    # Fallback: move into a local safe folder if Recycle Bin op fails
                    ...
                    try:
                        ...
                        deleted.append(path)  # treat as deleted for UI purposes
        ...
        # Update UI groups after delete/missing
